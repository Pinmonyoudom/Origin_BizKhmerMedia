...
const Caption = props => {
  ...
  const [facebookUrl, setFacebookUrl] = useState(facebook_link);
  const [instagramUrl, setinstagramUrl] = useState(instagram_link);
  const [twitterUrl, setTwitterUrl] = useState(twitter_link);
  const [linkedinUrl, setLinkedinUrl] = useState(linkedin_link);

  const updateLink = (id, field, value, setState) => {
    let data = '';
    data = window.prompt('Please enter icon URL', value);
    if (!(data == '' || data == null)) {
      setState(data);
      putData(id, field, data);
    }
  };

  useEffect(
    () => {
      setFacebookUrl(facebook_link);
      setinstagramUrl(instagram_link);
      setTwitterUrl(twitter_link);
      setLinkedinUrl(linkedin_link);
    },
              ...
              >
                <ul>
                  ...
                  >
                    ...
                    >
                      <p id="sub_caption">{sub_caption}</p>
                    </div>
                    {/* LAYER NR. 6 */}
                  </li>
                </ul>
